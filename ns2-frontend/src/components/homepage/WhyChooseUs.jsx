...
const WhyChooseUs = ({ data }) => {
  ...
  return (
    <section
      className="relative w-full mt-0 sm:mt-0 lg:mt-0 py-16 sm:py-20 overflow-hidden"
      style={{
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-2 gap-12 items-stretch relative z-10">
        {
          <div className="space-y-8 text-center lg:text-left flex flex-col justify-center animate-fade-in-up">
            {data.super_heading && (
              <h2
                className="font-semibold text-sm sm:text-base relative inline-block after:absolute after:-bottom-1 after:left-0 after:w-1/2 after:h-0.5 after:transition-all after:duration-500 hover:after:w-full"
                style={{ color: "#007BFF" }}
              >
                {data.super_heading}
              </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6">
              {data.content_items?.map((item, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="rounded-xl shadow-md p-6 hover:shadow-lg transition-all duration-300 flex flex-col gap-4 group hover:-translate-y-1 animate-fade-in-up"
                    style={{
